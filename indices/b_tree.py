...
def b_tree_main(path, page_size):
    ...
    for i in range(total_data_size):
        btree.insert(Item(data.iloc[i, 0], data.iloc[i, 1]))
    ...
    for x in range(test_data_size):
        ...
        btree.predict(data.iloc[num_index, 0])

    end_time = datetime.datetime.now()

    print("Time taken for B-Tree construction:\t" +
          str(model_end_time - model_start_time) + " seconds")
    print("Time taken for B-Tree lookup:\t" +
