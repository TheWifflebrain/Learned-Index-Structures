...
class AbstractNN:
    ...
    def predict(self, input_key):
        tmp_res = np.mat(input_key) * \
            np.mat(self.weights[0]) + np.mat(self.bias[0])
        for i in range(1, len(self.core_nums) - 1):
            tmp_res = np.mat(tmp_res) * \
                np.mat(self.weights[i]) + np.mat(self.bias[i])
...
class TrainedNN:
    def __init__(self, threshold, useThreshold, cores, train_step_num, batch_size, learning_rate, keep_ratio, train_x, train_y,
        ...
        self.y_ = tf.placeholder(
            tf.float32, shape=[None, self.core_nums[-1]])
        self.w_fc = []
        self.b_fc = []
